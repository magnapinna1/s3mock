package io.kipp.s3mock.alpakka
...
import io.kipp.s3mock.S3MockTest
...
class GetObjectTest extends S3MockTest {
  ...
  override def behaviour(fixture: => Fixture) = {
    ...
    it should "get objects via alpakka" in {
      ...
      val (result, _) = Await
        .result(
          fixture.alpakka.download("alpakka1", "test1").runWith(Sink.head),
          5.seconds
        )
        .get
      val str = Await
        .result(result.runFold(ByteString(""))(_ ++ _), 5.seconds)
        .utf8String
      str shouldBe "foobar"
    }
    ...
    it should "get by range" in {
      ...
      val (result, _) = Await
        .result(
          fixture.alpakka
            .download("alpakka2", "test2", Some(ByteRange(1, 4)))
            .runWith(Sink.head),
          5.seconds
        )
        .get
