...
class CopyObjectTest extends S3MockTest {
  override def behaviour(fixture: => Fixture) = {
    ...
    it should "copy an object even if destdir does not exist" in {
      ...
      getContent(
        s3.getObject("bucket-2", "folder/test.txt")
      ) shouldBe "contents"
    }
    ...
    it should "copy an object with new metadata" in {
      ...
      obj.getObjectMetadata.getUserMetadata.get(
        "new-key1"
      ) shouldBe "new-value1"
      obj.getObjectMetadata.getUserMetadata.get(
        "new-key2"
    ...
    it should "copy an object with = in key" in {
      ...
      val copyRequest = new CopyObjectRequest(
        "test-bucket",
        "path/with=123/test.txt",
        "test-bucket",
        "path/with=345/test2.txt"
      )
      s3.copyObject(copyRequest)
      getContent(
        s3.getObject("test-bucket", "path/with=345/test2.txt")
      ) shouldBe "contents"
    }
  }
}
