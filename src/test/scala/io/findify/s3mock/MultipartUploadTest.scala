...
class MultipartUploadTest extends S3MockTest {
  override def behaviour(fixture: => Fixture) = {
    ...
    it should "upload multipart files" in {
      ...
      val response1 = Await.result(
        http.singleRequest(
          HttpRequest(
            ...
            uri = s"http://127.0.0.1:$port/getput/foo2?uploads"
          )
        ),
        10.minutes
      )
      val data = Await.result(
        response1.entity.dataBytes
          .fold(ByteString(""))(_ ++ _)
          .runWith(Sink.head),
        10.seconds
      )
      val uploadId =
        (scala.xml.XML.loadString(data.utf8String) \ "UploadId").text
      val response2 = Await.result(
        http.singleRequest(
          HttpRequest(
            method = HttpMethods.POST,
            uri =
              s"http://127.0.0.1:$port/getput/foo2?partNumber=1&uploadId=$uploadId",
            entity = "foo"
          )
        ),
        10.minutes
      ...
      val response3 = Await.result(
        http.singleRequest(
          HttpRequest(
            method = HttpMethods.POST,
            uri =
              s"http://127.0.0.1:$port/getput/foo2?partNumber=2&uploadId=$uploadId",
            entity = "boo"
          )
        ),
        10.minutes
      ...
      val response4 = Await.result(
        http.singleRequest(
          HttpRequest(
            method = HttpMethods.POST,
            uri = s"http://127.0.0.1:$port/getput/foo2?uploadId=$uploadId",
            entity = commit
          )
        ),
        10.minutes
    ...
    it should "work with java sdk" in {
      ...
      val init = s3.initiateMultipartUpload(
        new InitiateMultipartUploadRequest("getput", "foo4")
      )
      val p1 = s3.uploadPart(
        new UploadPartRequest()
          .withBucketName("getput")
          .withPartSize(10)
          .withKey("foo4")
          .withPartNumber(1)
          .withUploadId(init.getUploadId)
          .withInputStream(new ByteArrayInputStream("hellohello".getBytes()))
      )
      val p2 = s3.uploadPart(
        new UploadPartRequest()
          .withBucketName("getput")
          .withPartSize(10)
          .withKey("foo4")
          .withPartNumber(2)
          .withUploadId(init.getUploadId)
          .withInputStream(new ByteArrayInputStream("worldworld".getBytes()))
      )
      val result = s3.completeMultipartUpload(
        new CompleteMultipartUploadRequest(
          ...
          "foo4",
          init.getUploadId,
          List(p1.getPartETag, p2.getPartETag).asJava
        )
      )
    ...
    it should "work with large blobs" in {
      val init = s3.initiateMultipartUpload(
        new InitiateMultipartUploadRequest("getput", "fooLarge")
      )
      val blobs = for (i <- 0 to 200) yield {
        ...
        val p1 = s3.uploadPart(
          new UploadPartRequest()
            .withBucketName("getput")
            .withPartSize(blob1.length)
            .withKey("fooLarge")
            .withPartNumber(i)
            .withUploadId(init.getUploadId)
            .withInputStream(new ByteArrayInputStream(blob1))
        )
      ...
      DigestUtils.md5Hex(
        s3.getObject("getput", "fooLarge").getObjectContent
      ) shouldBe DigestUtils.md5Hex(blobs.map(_._1).fold(Array[Byte]())(_ ++ _))
    }
    ...
    it should "upload multipart with metadata" in {
      ...
      val init = s3.initiateMultipartUpload(
        new InitiateMultipartUploadRequest("getput", "foo4", metadata)
      )
      val p1 = s3.uploadPart(
        new UploadPartRequest()
          .withBucketName("getput")
