...
class ChunkBuffer extends LazyLogging {
  ...
  def readHeader: Option[Header] = {
    ...
    if (
      (size.length <= 8) && (sig.length == 83) && sig.startsWith(
        ";chunk-signature="
      ) && sig.endsWith("\r\n")
    ) {
      val header = Header(
        Integer.parseInt(size.utf8String, 16),
        size.length + 83,
        sig.drop(17).dropRight(2).utf8String
      )
  ...
  def pullChunk(header: Header): Option[ByteString] = {
    if (buffer.length >= header.headerSize + header.chunkSize + 2) {
      ...
      logger.debug(
        s"not enough data to pull chunk: chunkSize = ${header.chunkSize}, bufferSize = ${buffer.length}"
      )
      None
    }
  }
