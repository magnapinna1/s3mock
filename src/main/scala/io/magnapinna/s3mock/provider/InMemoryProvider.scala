package io.magnapinna.s3mock.provider
...
import io.magnapinna.s3mock.error.{NoSuchBucketException, NoSuchKeyException}
import io.magnapinna.s3mock.provider.metadata.{InMemoryMetadataStore, MetadataStore}
import io.magnapinna.s3mock.request.{
  CompleteMultipartUpload,
  CreateBucketConfiguration
}
import io.magnapinna.s3mock.response._
...
class InMemoryProvider extends Provider with LazyLogging {
  ...
  private val multipartTempStore =
    new TrieMap[String, mutable.SortedSet[MultipartChunk]]

  private case class BucketContents(
      creationTime: DateTime,
      keysInBucket: mutable.Map[String, KeyContents]
  )

  private case class KeyContents(
      lastModificationTime: DateTime,
      data: Array[Byte]
  )

  private case class MultipartChunk(partNo: Int, data: Array[Byte])
      extends Ordered[MultipartChunk] {
    override def compare(that: MultipartChunk): Int =
      partNo compareTo that.partNo
  }

  override def metadataStore: MetadataStore = mdStore

  override def listBuckets: ListAllMyBuckets = {
    val buckets = bucketDataStore map { case (name, data) =>
      Bucket(name, data.creationTime)
    }
  ...
  override def listBucket(
    ...
    bucketDataStore.get(bucket) match {
      case Some(bucketContent) =>
        val matchingKeys =
          bucketContent.keysInBucket.filterKeys(_.startsWith(prefix2))
        val matchResults = matchingKeys map { case (name, content) =>
          Content(
            name,
            content.lastModificationTime,
            DigestUtils.md5Hex(content.data),
            content.data.length,
            "STANDARD"
          )
        }
        ...
        val commonPrefixes = normalizeDelimiter(delimiter) match {
          case Some(del) =>
            matchResults
              .flatMap(f => commonPrefix(f.key, prefix2, del))
              .toList
              .sorted
              .distinct
          case None => Nil
        }
        val filteredFiles: List[Content] = matchResults
  ...
  override def putObject(
    ...
    bucketDataStore.get(bucket) match {
      case Some(bucketContent) =>
        logger.debug(
          s"putting object for s3://$bucket/$key, bytes = ${data.length}"
        )
  ...
  override def putObjectMultipartPart(
    ...
    bucketDataStore.get(bucket) match {
      case Some(_) =>
        logger.debug(
          s"uploading multipart chunk $partNumber for s3://$bucket/$key"
        )
        multipartTempStore
          .getOrElseUpdate(uploadId, new mutable.TreeSet)
          .add(MultipartChunk(partNumber, data))
      case None => throw NoSuchBucketException(bucket)
    }
  }
  ...
  override def putObjectMultipartComplete(
    ...
    bucketDataStore.get(bucket) match {
      case Some(bucketContent) =>
        val completeBytes = multipartTempStore(uploadId).toSeq
          .map(_.data)
          .fold(Array[Byte]())(_ ++ _)
        bucketContent.keysInBucket.put(
          key,
          KeyContents(DateTime.now, completeBytes)
        )
      ...
      case (Some(srcBucketContent), Some(dstBucketContent)) =>
        srcBucketContent.keysInBucket.get(sourceKey) match {
          case Some(srcKeyContent) =>
            ...
            dstBucketContent.keysInBucket.put(
              destKey,
              KeyContents(destFileModTime, srcKeyContent.data.clone)
            )
            logger.debug(
              s"Copied s3://$sourceBucket/$sourceKey to s3://$destBucket/$destKey"
            )
            val sourceMeta =
              newMeta.orElse(metadataStore.get(sourceBucket, sourceKey))
            sourceMeta.foreach(meta =>
              metadataStore.put(destBucket, destKey, meta)
            )
            CopyObjectResult(
              destFileModTime,
              DigestUtils.md5Hex(srcKeyContent.data)
            )
        ...
        bucketContent.keysInBucket.get(key) match {
          ...
          case None =>
            bucketContent.keysInBucket.keys.find(_.startsWith(key)) match {
              case Some(_) =>
                logger.debug(
                  s"recursive delete by prefix is not supported by S3"
                )
                ()
              case None =>
