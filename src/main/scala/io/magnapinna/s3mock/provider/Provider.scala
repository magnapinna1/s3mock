...
import io.magnapinna.s3mock.provider.metadata.MetadataStore
...
trait Provider {
  ...
  def putObjectMultipartStart(
      bucket: String,
      key: String,
      metadata: ObjectMetadata
  ): InitiateMultipartUploadResult
  def putObjectMultipartPart(
      bucket: String,
      key: String,
      partNumber: Int,
      uploadId: String,
  ...
  def putObjectMultipartComplete(
      bucket: String,
      key: String,
      uploadId: String,
  ...
  def normalizeDelimiter(delimiter: Option[String]): Option[String] =
    delimiter.flatMap { s => if (s.isEmpty) None else Some(s) }
}
