...
object MetadataUtil extends LazyLogging {
  ...
  def populateObjectMetadata(request: HttpRequest): ObjectMetadata = {
    ...
    for (header <- headers.asScala) {
      ...
      if (
        StringUtils.beginsWithIgnoreCase(key, Headers.S3_USER_METADATA_PREFIX)
      ) {
      ...
      //      }
        ...
        try
          ...
          case pe: Exception =>
            logger.warn(
              "Unable to parse last modified date: " + header.value(),
              pe
            )
        }
        ...
        metadata.setHeader(key, ServiceUtils.removeQuotes(header.value()))
        ...
        try
          ...
          case pe: Exception =>
            logger.warn(
              "Unable to parse http expiration date: " + header.value(),
              pe
            )
        }
