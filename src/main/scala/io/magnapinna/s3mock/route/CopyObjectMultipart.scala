...
import io.magnapinna.s3mock.error.{
...
case class CopyObjectMultipart()(implicit provider: Provider)
    extends LazyLogging {
  ...
  def extractMetadata(req: HttpRequest): Option[ObjectMetadata] = {
    req.headers
      .find(_.lowercaseName() == "x-amz-metadata-directive")
      .map(_.value()) match {
      case Some("REPLACE") =>
  ...
  def route(destBucket: String, destKey: String) =
    parameter('partNumber, 'uploadId) {
        ...
        put {
          headerValueByName("x-amz-copy-source") { source =>
            extractRequest { req =>
              complete {
                val byteSeq =
                  req.getHeader("x-amz-copy-source-range").get().value()
                    ...
                    Try(
                      ...
                      case Success(result) =>
                        ...
                        HttpResponse(
                          ...
                          entity = result.toXML.toString()
                        )
                      case Failure(e: NoSuchKeyException) =>
                        logger.info(
                          s"cannot copy object $sourceBucket/$sourceKey: no such key"
                        )
                        HttpResponse(
                      ...
                      case Failure(e: NoSuchBucketException) =>
                        logger.info(
                          s"cannot copy object $sourceBucket/$sourceKey: no such bucket"
                        )
                        HttpResponse(
                          StatusCodes.NotFound,
                          entity = e.toXML.toString()
                      ...
                      case Failure(t) =>
                        logger.error(
                          s"cannot copy object $sourceBucket/$sourceKey: $t",
                          t
                        )
                        HttpResponse(
