...
import io.magnapinna.s3mock.S3ChunkedProtocolStage
import io.magnapinna.s3mock.error.{InternalErrorException, NoSuchBucketException}
import io.magnapinna.s3mock.provider.Provider
import org.apache.commons.codec.digest.DigestUtils

import scala.util.{Failure, Success, Try}

case class PutObjectMultipart()(implicit provider: Provider, mat: Materializer)
    extends LazyLogging {
  ...
  def route(bucket: String, path: String) = parameter('partNumber, 'uploadId) {
    ...
    extractRequest { request =>
      complete {
        val result = request.entity.dataBytes
          ...
          .map(data => {
            Try(
              provider.putObjectMultipartPart(
                bucket,
                path,
                partNumber.toInt,
                uploadId,
